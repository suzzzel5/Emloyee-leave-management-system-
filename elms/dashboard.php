    ...
    <body>
       ...
       <?php include('includes/sidebar.php');?>
            ...
            <main class="mn-inner">
                <div class="">
<?php
// Notification bar for latest approved / not approved leaves for this employee
$eid=$_SESSION['eid'];
$sqlNotif = "SELECT LeaveType, PostingDate, Status FROM tblleaves WHERE empid = :eid AND Status IN (1,2) ORDER BY id DESC LIMIT 5";
$queryNotif = $dbh->prepare($sqlNotif);
$queryNotif->bindParam(':eid',$eid,PDO::PARAM_STR);
$queryNotif->execute();
$notifications = $queryNotif->fetchAll(PDO::FETCH_OBJ);
if($queryNotif->rowCount() > 0)
{
?>
                    <div class="row no-m-t">
                        <div class="col s12">
<?php foreach($notifications as $n) {
    $label = ($n->Status==1) ? 'Approved' : 'Not Approved';
    $colorClass = ($n->Status==1) ? 'green lighten-4' : 'red lighten-4';
    $textColor = ($n->Status==1) ? 'green-text text-darken-2' : 'red-text text-darken-2';
?>
                            <div class="card-panel <?php echo $colorClass; ?>">
                                <span class="<?php echo $textColor; ?>">
                                    Your <?php echo htmlentities($n->LeaveType);?> leave applied on <?php echo htmlentities($n->PostingDate);?> has been <strong><?php echo $label; ?></strong> by admin.
                                </span>
                            </div>
<?php } ?>
                        </div>
